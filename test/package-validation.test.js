...
test('package validator confirms required deployable artifacts', () => {
  ...
  assert.equal(report.version, '0.7.17');
...
test('README documents native path routing and separate protocol keys', () => {
  ...
  assert.match(readme, /CLAUDE_CODE_OUTPUT_REQUIRED_RECOVERY_DISABLE_THINKING/);
  assert.match(readme, /CLAUDE_CODE_ACTION_REQUIRED_RECOVERY_DISABLE_THINKING/);
  assert.match(readme, /CLAUDE_CODE_FORCED_TOOL_RECOVERY_MAX_TEXT_BYTES/);
  assert.match(readme, /forced_tool_recovery_excess_text/);
  assert.match(readme, /think=false.*enable_thinking=false|enable_thinking=false.*think=false/is);
