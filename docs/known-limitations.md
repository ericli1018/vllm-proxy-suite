...
- v0.7.1 manages one or more homogeneous WebSearch calls or one or more homogeneous WebFetch calls with bounded concurrency. Mixed Managed/Client Tool batches are still delivered unchanged to Claude Code.
...
- The bridge buffers each vLLM sub-response and performs an internal continuation, increasing latency and context usage. Completion pings keep streaming connections active, but the model continuation still waits for every Tool Result belonging to the same assistant turn.
...
- Managed Web Tool activity is internal to the Proxy and is not rendered as a Claude Code Tool row. Per-item SSE ping frames are transport progress only and normally produce no visible Claude Code text.
