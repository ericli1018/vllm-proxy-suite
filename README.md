...
`WebFetch` 使用相同排程，但每一項可能包含下載、Chunk Reader 與 Synthesizer，因此由 `WEBFETCH_MAX_PARALLEL` 獨立限制。Search 與 Fetch 的使用上限分別追蹤：WebFetch 達上限只移除 Fetch，後續 WebSearch 仍可繼續；WebSearch 達上限只移除 Search，後續 WebFetch 仍可繼續。若模型在達上限後再次呼叫已移除的 Managed Tool，Proxy 先給唯一一次 bounded finalization continuation，回傳 `is_error:true` Tool Result，明確要求使用既有證據並停止呼叫該 Tool；若模型在這次 finalization 後仍再次呼叫，才以 `managed_web_tool_limit_repeated`、`retryable:false` 熔斷。此路徑最多增加一次 vLLM continuation，不存在第三次 finalization。研究型工作可依需求明確將 `WEBFETCH_MAX_USES` 設為 `6` 或 `8`，但仍保持固定上限。

Managed Tool 分類會先把「存在完整 Tool Call、但上游錯誤回 `end_turn`」視為 `tool_use`，避免 Hosted `web_search` 被上層才修正而逃逸。若同一平行批次混合 `WebSearch`／`WebFetch` 與 `TaskUpdate`、`Bash` 等 Client Tool，Proxy 只允許一次序列化：先執行 Managed Web Tool，其他 Tool Call 不執行並要求模型在下一輪重新發出；第二次仍混合則以 `managed_web_mixed_batch_repeated`、`retryable:false` 熔斷。已適配的 Hosted `web_search` 若無法被安全解析或管理，會以 `managed_hosted_tool_escape` 熔斷，絕不 replay 給 Claude Code。所有非重試熔斷在尚未送出回應標頭時使用 HTTP `422`，Managed SSE 已開始時則送出等價 `event:error`，並記錄 `client_retry_suppressed`。

Proxy 不會把內部 `tool_result` 暴露給 Claude Code，也不會顯示成 `Search(...)` 工具列。只有確定進入 Managed WebSearch／WebFetch 後才建立 synthetic `message_start`；普通 Bash、Read、Write 或純文字回應維持原始 Anthropic stream，不會多出空白 `●`。WebSearch 預設顯示清理後的 query；WebFetch 只顯示 hostname，不顯示 URL path、query string、token、Cookie、snippet 或正文。週期性可見 `text_delta` 會持續追加省略號，最後將 vLLM SSE content block index 平移並 splice 到同一 lifecycle。Progress block 以不可見 sentinel 標記，下一輪送往 vLLM 前會整塊移除，避免污染模型歷史與 prefix cache。Anthropic Tool ID 配對仍要求同一 assistant turn 的所有結果齊備後才能安全 continuation。因 synthetic `message_start` 必須在取得 upstream usage 前送出，Claude Code 端的 `usage.input_tokens` 會是 `0`；Proxy Log 仍保留 vLLM 最終回報的實際 token usage。
